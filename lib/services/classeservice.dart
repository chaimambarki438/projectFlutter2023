...
import 'package:flutterproject/entities/classe.dart';
...
Future<List<Matier>> getAllMatiers() async {
  Response response =
      await http.get(Uri.parse("http://10.0.2.2:8080/matier/all"));
  List<dynamic> jsonResponse = jsonDecode(response.body);
...
Future deleteAbsence(int? id) {
  return http.delete(Uri.parse("http://10.0.2.2:8080/absence/delete?id=${id}"));
}

Future addMatier(Matier matier, int classId) async {
  Response response = await http.post(
      Uri.parse("http://10.0.2.2:8080/matier/add/$classId"),
      headers: {"Content-type": "Application/json"},
      body: jsonEncode(<String, dynamic>{
...
Future updateMatier(Matier matier) async {
  Response response =
      await http.put(Uri.parse("http://10.0.2.2:8080/matier/update"),
          headers: {"Content-type": "Application/json"},
          body: jsonEncode(<String, dynamic>{
...
Future deleteMatier(int id) {
  return http.delete(Uri.parse("http://10.0.2.2:8080/matier/delete?id=${id}"));
}
